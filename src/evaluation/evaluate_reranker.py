...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate only the reranked retriever with ground-truth RAGAS metrics."
    )
    ...
    parser.add_argument(
        ...
        help="Number of initial candidate chunks retrieved before reranking.",
    )

    parser.add_argument(
        "--rerank-top-n",
        type=int,
        default=config.RERANK_TOP_N,
        help="Number of top chunks kept after reranking.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=EVAL_FILE,
        help="Path to the ground-truth evaluation file.",
    )
...
async def main():
    ...
    # Override config values for this run only.
    # This lets you run:
    #   python src/evaluation/evaluate_reranked_only.py --retriever-k 20 --rerank-top-n 3
    #   python src/evaluation/evaluate_reranked_only.py --retriever-k 30 --rerank-top-n 3
    #   python src/evaluation/evaluate_reranked_only.py --retriever-k 40 --rerank-top-n 3
    config.RETRIEVER_K = args.retriever_k
    config.RERANK_TOP_N = args.rerank_top_n

    run_name = f"reranked_k{args.retriever_k}_top{args.rerank_top_n}"
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    output_file = output_dir / f"{run_name}_full.csv"
    summary_file = output_dir / f"{run_name}_summary.csv"
    category_file = output_dir / f"{run_name}_by_category.csv"

    eval_rows = load_eval_dataset(args.eval_file)

    print(f"Loaded {len(eval_rows)} ground-truth evaluation rows")
    print(f"RUNNING SETUP: {run_name}")
    print(
        {
    ...
    if "category" in df.columns and df["category"].notna().any():
        ...
        category_summary.to_csv(category_file, index=False, encoding="utf-8-sig")
